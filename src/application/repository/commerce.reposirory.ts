...
export class CommerceRepository implements iRepository<Commerce> {
    ...
    async get(): Promise<Commerce[]> {
        return new Promise<Commerce[]>(async (resolve, reject) => {
            try {
                const commercesQueryResult = await firebaseDB.collection("commerces").get();
                const commerces = commercesQueryResult.docs.map<Commerce>(c => <Commerce>c.data())
                resolve(commerces);
            } catch (error) {
                reject(error)
            }
    ...
    async getById(commercePhoneNumber: string | number) {
        return new Promise<Commerce[]>(async (resolve, reject) => {
            try {
                ...
                const commerces = await commercesQueryResult
                    ...
                    .get();
                if (!!commerces.size) {
                    reject('commerce_no_fount')
                    return
                }
                const commercesData = commerces.docs.map<Commerce>(commerce => <Commerce>commerce.data());
    ...
    async create(commerceInfo: Commerce): Promise<string> {
        return new Promise<string>(async (resolve, reject) => {
            try {
                const commercesQueryResult = await firebaseDB.collection("commerces").add(commerceInfo);
        ...
        return new Promise<OperationStatus>(async (resolve, reject) => {
            try {
                ...
                if (!!commercesQueryResult.docs.length) {
                    await commercesQueryResult.docs[commercesQueryResult.docs.length].ref.set(data);
                    resolve("Completed");
                }
                resolve("NoCompleted");
            } catch (error) {
    ...
    async deactivate(commercePhoneNumber: string | number): Promise<OperationStatus> {
        return new Promise<OperationStatus>(async (resolve, reject) => {
            try {
                const commercesQueryResult = await firebaseDB.collection("commerces")
                    .where("phone", "==", commercePhoneNumber)
                    .get();
                if (!!commercesQueryResult.docs.length) {
                    await commercesQueryResult.docs[commercesQueryResult.docs.length].ref.set({ enable: false }) // .delete();
                    resolve("Completed");
                }
                resolve("NoCompleted");
            } catch (error) {
